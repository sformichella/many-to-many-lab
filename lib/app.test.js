...
describe('artist routes', () => {
  ...
  it('finds an artist by id and returns all associated albums', async() => {
    ...
    expect(response.body.artist).toEqual('Gift of Gab');
    expect(response.body.id).toEqual('1');
    expect(response.body.albums).toEqual(expect.arrayContaining(albumNames));
    expect(response.body.albums.length).toEqual(albumNames.length);
  });
...
describe('album routes', () => {
  ...
  it('updates an album', async() => {
    ...
    const updatedAlbum = { id: '1', album: 'The Blazing Arrow' };
